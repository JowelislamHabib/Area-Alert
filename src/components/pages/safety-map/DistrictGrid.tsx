...
export function DistrictGrid({ stats, activeTab, onViewAreas }: DistrictGridProps) {
  ...
  const renderCard = (
    ...
    return (
      <div key={key} className="group relative bg-card rounded-2xl border border-border shadow-sm p-5 hover:shadow-md hover:shadow-primary/5 hover:border-primary/20 hover:-translate-y-0.5 transition-all duration-300 flex flex-col">
        
        {/* Header */}
        <div className="flex justify-between items-start mb-4">
          <h3 className="font-bold text-lg flex items-center gap-2 group-hover:text-primary transition-colors">
            <MapPin className="w-4 h-4 text-muted-foreground" />
            {title} {data.district && !isDistrict ? <span className="text-xs text-muted-foreground font-normal">({data.district})</span> : null}
          </h3>
          <div className={cn("px-2.5 py-1 rounded-md text-xs font-semibold flex items-center", getBadgeStyle(data.safetyLevel))}>
        ...
        <div className="text-xs text-muted-foreground mb-5 flex items-center gap-2">
          ...
          <span className={data.activeReports > 0 ? "text-destructive font-semibold" : "text-emerald-600 font-semibold"}>
        ...
        <div className="grid grid-cols-5 gap-2 mb-5">
          <div className="bg-amber-500/5 hover:bg-amber-500/10 transition-colors rounded-xl p-3 flex flex-col items-center justify-center border border-amber-500/20">
            ...
            <span className="text-[10px] text-amber-600/70 dark:text-amber-400/70 font-medium">Elec</span>
          </div>
          <div className="bg-blue-500/5 hover:bg-blue-500/10 transition-colors rounded-xl p-3 flex flex-col items-center justify-center border border-blue-500/20">
            ...
            <span className={cn("font-bold", data.activeUtilities?.water > 0 ? "text-blue-600 dark:text-blue-400" : "text-blue-600/50 dark:text-blue-400/50")}>{data.activeUtilities?.water || 0}</span>
            <span className="text-[10px] text-blue-600/70 dark:text-blue-400/70 font-medium">Wate</span>
          </div>
          <div className="bg-indigo-500/5 hover:bg-indigo-500/10 transition-colors rounded-xl p-3 flex flex-col items-center justify-center border border-indigo-500/20">
            ...
            <span className={cn("font-bold", data.activeUtilities?.internet > 0 ? "text-indigo-600 dark:text-indigo-400" : "text-indigo-600/50 dark:text-indigo-400/50")}>{data.activeUtilities?.internet || 0}</span>
            <span className="text-[10px] text-indigo-600/70 dark:text-indigo-400/70 font-medium">Inte</span>
          </div>
        ...
        <div className="flex justify-between items-center mt-auto pt-5">
          ...
          <Button variant="ghost" size="sm" className="gap-1.5 text-muted-foreground hover:text-foreground hover:bg-secondary" asChild>
            <Link href={`/reports?${isDistrict ? 'district' : 'area'}=${encodeURIComponent(title)}`}>
