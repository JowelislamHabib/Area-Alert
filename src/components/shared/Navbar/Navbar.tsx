...
import { LogOut, Menu, User, Plus, FileText, LayoutDashboard } from "lucide-react";
...
export default function Navbar() {
  ...
  const callbackQuery = isAuthPage ? '' : `?redirect=${encodeURIComponent(currentPath)}`;
    ...
    <header className="sticky top-0 z-50 w-full border-b bg-[#F4F7F6] dark:bg-[#0F172A]">
      <div className="mx-auto flex h-14 max-w-7xl items-center justify-between px-4 sm:px-6 lg:px-8">
        ...
        <div className="hidden lg:flex items-center gap-2">
          ...
          {isPending ? (
            ...
            <div className="flex items-center gap-2 pl-2">
              <Button asChild className="font-semibold rounded-md">
                ...
                <DropdownMenuContent align="end" sideOffset={8} className="w-56 rounded-xl">
                  ...
                  <DropdownMenuGroup>
                    <DropdownMenuItem asChild className="cursor-pointer py-2.5">
                      <Link href="/reports/my-reports" className="flex items-center w-full">
                  ...
                  <DropdownMenuSeparator />
                  <DropdownMenuItem onClick={handleLogout} variant="destructive" className="cursor-pointer py-2.5 flex items-center w-full text-destructive focus:bg-destructive/10 focus:text-destructive">
            ...
            <div className="flex items-center gap-2 ml-2 pl-4 border-l border-border/40">
              <Button asChild variant={pathname === '/login' ? 'default' : 'ghost'} className={`font-semibold rounded-md ${pathname === '/login' ? '' : 'text-muted-foreground hover:text-foreground'}`}>
                <Link href={`/login${callbackQuery}`} className="flex items-center">
                  Login
                </Link>
              </Button>
              <Button asChild variant={pathname === '/register' ? 'default' : 'outline'} className="font-semibold rounded-md">
                <Link href={`/register${callbackQuery}`} className="flex items-center">
                  Register
                </Link>
              </Button>
            </div>
        ...
        <div className="flex lg:hidden items-center gap-3">
          {user && !isPending && (
            <Button asChild size="sm" className="font-semibold rounded-md h-8 px-3">
              <Link href="/add-report" className="flex items-center">
          ...
          <SheetContent side="right" className="w-[85vw] sm:w-80 flex flex-col p-0 border-l border-border/50">
            ...
            <div className="flex-1 overflow-y-auto">
              ...
              {user && (
                <div className="px-4 pb-4">
                  ...
                  <h4 className="text-xs font-bold uppercase tracking-wider text-muted-foreground mb-3 px-2">Account</h4>
            ...
            <div className="p-4 border-t border-border/40 bg-muted/20">
              {user ? (
                <div className="flex flex-col gap-3">
                  ...
                  <Button asChild className="w-full rounded-md font-semibold mt-2" onClick={() => setOpen(false)}>
                    <Link href="/add-report" className="flex items-center justify-center">
                      <Plus className="mr-1.5 size-4" /> Add Report
                    </Link>
                  </Button>
                ...
                <div className="flex flex-col gap-2">
                  <Button asChild variant={pathname === '/register' ? 'default' : 'outline'} className="w-full rounded-md font-semibold" onClick={() => setOpen(false)}>
                    <Link href={`/register${callbackQuery}`} className="flex items-center justify-center">
                      Register
                    </Link>
                  </Button>
                  <Button asChild variant={pathname === '/login' ? 'default' : 'outline'} className="w-full rounded-md font-semibold" onClick={() => setOpen(false)}>
                    <Link href={`/login${callbackQuery}`} className="flex items-center justify-center">
                      Login
                    </Link>
                  </Button>
                </div>
