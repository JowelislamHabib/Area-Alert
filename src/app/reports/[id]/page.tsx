...
const UTILITY_ICONS = {
  electricity: { icon: Zap, color: "text-yellow-600 dark:text-yellow-400", bg: "bg-yellow-100 dark:bg-yellow-500/10", label: "Electricity" },
  internet: { icon: Wifi, color: "text-purple-600 dark:text-purple-400", bg: "bg-purple-100 dark:bg-purple-500/10", label: "Internet" },
  water: { icon: Droplets, color: "text-cyan-600 dark:text-cyan-400", bg: "bg-cyan-100 dark:bg-cyan-500/10", label: "Water" },
  gas: { icon: Flame, color: "text-orange-600 dark:text-orange-400", bg: "bg-orange-100 dark:bg-orange-500/10", label: "Gas" },
};
          ...
          <div className="p-5 sm:p-6 lg:p-8 relative z-10">
            <div className="flex flex-wrap items-center gap-3 mb-5 mt-1">
              ...
              <Badge variant="outline" className={`border-none rounded-full px-3 py-1 font-bold text-[10px] uppercase tracking-wider gap-1.5 ${report.status === "active" ? "bg-red-50 text-red-600 dark:bg-red-500/10 dark:text-red-400" : report.status === "resolved" ? "bg-emerald-50 text-emerald-600 dark:bg-emerald-500/10 dark:text-emerald-400" : "bg-muted text-muted-foreground"}`}>
                <div className={`size-2 rounded-full animate-pulse ${report.status === "active" ? "bg-red-500" : report.status === "resolved" ? "bg-emerald-500" : "bg-muted-foreground"}`} />
                {report.status}
              </Badge>
            </div>
              ...
              <div className="p-5">
                <div className="space-y-4">
                  <div className="flex justify-between items-center py-2 border-b border-border/30 last:border-0">
                    <p className="text-[11px] font-bold text-muted-foreground uppercase tracking-wider">Utility Type</p>
                    <p className="text-[13px] font-semibold text-foreground capitalize">{report.utilityType}</p>
                  </div>
                  
                  {report.ispName && (
                    <div className="flex justify-between items-center py-2 border-b border-border/30 last:border-0">
                      <p className="text-[11px] font-bold text-muted-foreground uppercase tracking-wider">ISP Name</p>
                      <p className="text-[13px] font-semibold text-foreground">{report.ispName}</p>
                    </div>
                  )}

                  <div className="flex justify-between items-center py-2 border-b border-border/30 last:border-0">
                    <p className="text-[11px] font-bold text-muted-foreground uppercase tracking-wider">Area</p>
                    <p className="text-[13px] font-semibold text-foreground text-right max-w-[150px] truncate" title={report.area}>{report.area}</p>
                  </div>

                  <div className="flex justify-between items-center py-2 border-b border-border/30 last:border-0">
                    <p className="text-[11px] font-bold text-muted-foreground uppercase tracking-wider">District</p>
                    <p className="text-[13px] font-semibold text-foreground">{report.district}</p>
                  </div>

                  <div className="flex justify-between items-center py-2 border-b border-border/30 last:border-0">
                    <p className="text-[11px] font-bold text-muted-foreground uppercase tracking-wider">Status</p>
                    <p className="text-[13px] font-semibold text-foreground capitalize">{report.status}</p>
                  </div>

                  <div className="flex justify-between items-center py-2 border-b border-border/30 last:border-0">
                    <p className="text-[11px] font-bold text-muted-foreground uppercase tracking-wider">Started At</p>
                    <p className="text-[13px] font-semibold text-foreground">{format(new Date(report.startedAt), "MMM d, yyyy h:mm a")}</p>
                  </div>
                </div>
              </div>
            </Card>
            ...
            <Card className="shadow-sm p-0 gap-0 border-border/50 overflow-hidden">
              ...
              <div className="p-5">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <p className="text-[14px] font-bold text-foreground">{report.reporterName}</p>
